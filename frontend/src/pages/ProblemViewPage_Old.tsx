...
const ProblemViewPage: React.FC = () => {
  ...
  const loadProblem = async (id: number) => {
    try {
      ...
      // Use the new public API to get problem with sample test cases
      const response = await apiService.getProblemPublic(id);
      if (response.success && response.data) {
        const problemData = response.data;
        setProblem({
          id: problemData.id,
          contestId: problemData.contest_id,
          problemLetter: problemData.problem_letter,
          ...
          // Use the first sample test case for sampleInput/sampleOutput
          sampleInput: problemData.sample_test_cases?.[0]?.input || '',
          sampleOutput: problemData.sample_test_cases?.[0]?.expected_output || '',
          // Store all sample test cases for display
        ...
        <Card sx={{ mb: 3 }}>
          <CardContent>
            ...
            <Collapse in={expandedSections.samples}>
              {(problem as any)?.sampleTestCases?.length > 0 ? (
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {(problem as any).sampleTestCases.map((testCase: any, index: number) => (
                    <Box key={index}>
                        ...
                        <Paper variant="outlined" sx={{ p: 2 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
                            ...
                            <Tooltip title="Copy to clipboard">
                              <IconButton 
                                size="small" 
                                onClick={() => copyToClipboard(testCase.expected_output, 'output')}
                              >
                                <ContentCopy fontSize="small" />
                              </IconButton>
                            </Tooltip>
                ...
                <Alert severity="info" sx={{ mt: 2 }}>
                  No sample test cases are available for this problem. Please refer to the problem description and constraints for guidance.
                </Alert>
              )}
            </Collapse>
          </CardContent>
