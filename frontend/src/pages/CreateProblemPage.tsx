...
const CreateProblemPage: React.FC = () => {
  ...
  // Generate IO wrapper for each language
  const generateIOWrapper = (language: 'cpp' | 'java' | 'python'): string => {
    const functionSignature = generatePreview(language);
...
${formData.parameters.map((p, i) => {
  const langType = getLanguageType(p.type, language);
  return `    ${langType} ${p.name} = inputJson[${i}].get<${langType}>();`;
}).join('\n')}

    // Call user function
    auto result = ${formData.functionName}(${formData.parameters.map(p => p.name).join(', ')});
...
class Solution {
    ...
    public static void main(String[] args) {
        ...
        Gson gson = new Gson();
        JsonArray inputJson = gson.fromJson(line, JsonArray.class);

        // Extract parameters from JSON
${formData.parameters.map((p, i) => {
  const langType = getLanguageType(p.type, language);
  return `        ${langType} ${p.name} = gson.fromJson(inputJson.get(${i}), ${langType}.class);`;
}).join('\n')}

        // Call user function
        Solution solution = new Solution();
        var result = solution.${formData.functionName}(${formData.parameters.map(p => p.name).join(', ')});
...
if __name__ == "__main__":
    ...
    # Extract parameters
${formData.parameters.map((p, i) => `    ${p.name} = input_data[${i}]`).join('\n')}

    # Call user function
    result = ${formData.functionName}(${formData.parameters.map(p => p.name).join(', ')})
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const problemData = {
        ...
        // Function signatures
        function_signature_cpp: generatePreview('cpp'),
        function_signature_java: generatePreview('java'),
        function_signature_python: generatePreview('python'),
        // IO wrappers
        io_wrapper_cpp: generateIOWrapper('cpp'),
        io_wrapper_java: generateIOWrapper('java'),
        io_wrapper_python: generateIOWrapper('python')
      };
